package GNNC.modules.financial.report
{
	...
	public class gnncFileReport_financial_safeBalanceX extends gnncFileReport
	{
		public function gnncFileReport_financial_safeBalanceX()
		{
		}
		...
		override protected function __itemRender(object_:Object,i_:uint,objectAlternative_:Object=null):void
		{
			...
			{
				_gnncFilePdf.__setFontStyle(13,0x000000);
				_gnncFilePdf.__addCell(String(object_.MIX).indexOf("OUT")<0?"BALANÇO - RECEITAS":"BALANÇO - DESPESAS",260,"C",10);
				_gnncFilePdf.__breakLine(10);
				MIX = object_.MIX;
			}
		...
		override protected function __setValues():void
		{
			...
			{
				_sql += " (Select round(SUM(VALUE_IN_PAY)-SUM(VALUE_OUT_PAY),2) 	from dbd_financial where DATE_END >= '"+_dateStart.fullYear+"-"+gnncDataNumber.__setZero(e)+"-01 00:00:00' AND DATE_END <= '"+_dateStart.fullYear+"-"+gnncDataNumber.__setZero(e)+"-31 23:59:59' AND MIX <> 'FINANCIAL_TRANS' AND ID_GROUP like gro.ID	" +
					//DELETE
					"AND ID <> '125' AND ID <> '113' " +
					") as VALUE_TOTAL_MONTH_"+e+", ";
				_sql += " (Select round(SUM(VALUE_IN_PAY)-SUM(VALUE_OUT_PAY),2) 	from dbd_financial where DATE_END >= '"+_dateStart.fullYear+"-"+gnncDataNumber.__setZero(e)+"-01 00:00:00' AND DATE_END <= '"+_dateStart.fullYear+"-"+gnncDataNumber.__setZero(e)+"-31 23:59:59' AND MIX <> 'FINANCIAL_TRANS'								" +
					//DELETE
					"AND ID <> '125' AND ID <> '113' " +
					") as VALUE_TOTAL_MONTH_"+e+"_TOTAL, ";
			}
			
			_sql += "" +
				"(Select round(SUM(VALUE_IN_PAY)-SUM(VALUE_OUT_PAY),2) 	from dbd_financial where DATE_END >= '"+_dateStart.fullYear+"-01-01 00:00:00' AND DATE_END <= '"+_dateStart.fullYear+"-12-31 23:59:59' AND MIX <> 'FINANCIAL_TRANS' AND ID_GROUP like gro.ID	" +
				//DELETE
				"AND ID <> '125' AND ID <> '113' " +
				") as VALUE_TOTAL_MONTH_"+0+", "+
				"(Select round(SUM(VALUE_IN_PAY)-SUM(VALUE_OUT_PAY),2) 	from dbd_financial where DATE_END >= '"+_dateStart.fullYear+"-01-01 00:00:00' AND DATE_END <= '"+_dateStart.fullYear+"-12-31 23:59:59' AND MIX <> 'FINANCIAL_TRANS' 							" +
				//DELETE
				"AND ID <> '125' AND ID <> '113' " +
				") as VALUE_TOTAL_MONTH_"+0+"_TOTAL "+
				
				"from "+ 
				"dbd_group as gro "+
				
				"where " +
				"MIX like 'FINANCIAL_OUT' OR  MIX like 'FINANCIAL_IN' " +
				"AND MIX <> 'FINANCIAL_TRANS' "+
