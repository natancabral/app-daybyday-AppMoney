package GNNC.modules.financial.report
{
	...
	public class gnncFileReport_financial_addToday extends gnncFileReport_financial_today
	{
		public function gnncFileReport_financial_addToday(csv_:Boolean=false)
		{
			_csvReturn = csv_;
		}
		...
		override protected function __setValues():void
		{
			...
			_fileName 		= "DAYBYDAY - MONEY - Lancamentos Cadastrados no Dia";
			_documentTitle 	= "Lançamentos pela data de Cadastrado "+dateLegend;
			...
			var _WHERE:String = " " +
				"LEFT(DATE,10) >= LEFT('"+dateStartCurrent+" ',10) AND " +
				"LEFT(DATE,10) <= LEFT('"+dateEndCurrent+"   ',10) " + 
				"" + _filter;
			...
			//var ord:Array = ['FININ desc','DATE_END','NUMBER_LETTER','NUMBER','DATE_FINAL'];
			var ord:Array = [(_object['orderBy']?_object['orderBy']+',':'')+'FININ desc','DATE','ID_FINANCIAL_ACCOUNT','NUMBER_LETTER','NUMBER','DATE_FINAL asc','DATE_END'];
