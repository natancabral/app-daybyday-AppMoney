package GNNC.modules.financial.report
{
	...
	public class gnncFileReport_financial_safeOutX extends gnncFileReport
	{
		public function gnncFileReport_financial_safeOutX()
		{
		}
		...
		override protected function __setValues():void
		{
			...
			{
				_sql += " (Select round(SUM(VALUE_OUT_PAY),2) 	from dbd_financial where DATE_END >= '"+_dateStart.fullYear+"-"+gnncDataNumber.__setZero(e)+"-01 00:00:00' AND DATE_END <= '"+_dateStart.fullYear+"-"+gnncDataNumber.__setZero(e)+"-31 23:59:59' AND MIX <> 'FINANCIAL_TRANS' AND ID_GROUP like gro.ID	" +
					//DELETE
					"AND ID <> '125' AND ID <> '113' " +
					") as VALUE_TOTAL_MONTH_"+e+", ";
				_sql += " (Select round(SUM(VALUE_OUT_PAY),2) 	from dbd_financial where DATE_END >= '"+_dateStart.fullYear+"-"+gnncDataNumber.__setZero(e)+"-01 00:00:00' AND DATE_END <= '"+_dateStart.fullYear+"-"+gnncDataNumber.__setZero(e)+"-31 23:59:59' AND MIX <> 'FINANCIAL_TRANS' 							" +
					//DELETE
					"AND ID <> '125' AND ID <> '113' " +
			...
			_sql += "" +
				"(Select round(SUM(VALUE_OUT_PAY),2) 	from dbd_financial where DATE_END >= '"+_dateStart.fullYear+"-01-01 00:00:00' AND DATE_END <= '"+_dateStart.fullYear+"-12-31 23:59:59' AND MIX <> 'FINANCIAL_TRANS' AND ID_GROUP like gro.ID	" +
				//DELETE
				"AND ID <> '125' AND ID <> '113' " +
				") as VALUE_TOTAL_MONTH_"+0+", "+
				"(Select round(SUM(VALUE_OUT_PAY),2) 	from dbd_financial where DATE_END >= '"+_dateStart.fullYear+"-01-01 00:00:00' AND DATE_END <= '"+_dateStart.fullYear+"-12-31 23:59:59' AND MIX <> 'FINANCIAL_TRANS' 							" +
				//DELETE
				"AND ID <> '125' AND ID <> '113' " +
