package GNNC.modules.financial.report
{
	...
	public class gnncFileReport_financial_safeOut extends gnncFileReport
	{
		public function gnncFileReport_financial_safeOut()
		{
		}
		...
		override protected function __setValues():void
		{
			...
			_fileName = "DAYBYDAY - MONEY - Despesas";
			_documentTitle = "Despesas - "+_dateStart.fullYear;
			...
			{
				_sql += " (Select round(SUM(VALUE_OUT_PAY),2) 	from dbd_financial where DATE_FINAL >= '"+_dateStart.fullYear+"-"+gnncDataNumber.__setZero(e)+"-01 00:00:00' AND DATE_FINAL <= '"+_dateStart.fullYear+"-"+gnncDataNumber.__setZero(e)+"-31 23:59:59' AND MIX <> 'FINANCIAL_TRANS' AND ID_GROUP = gro.ID	) as VALUE_TOTAL_MONTH_"+e+", ";
				_sql += " (Select round(SUM(VALUE_OUT_PAY),2) 	from dbd_financial where DATE_FINAL >= '"+_dateStart.fullYear+"-"+gnncDataNumber.__setZero(e)+"-01 00:00:00' AND DATE_FINAL <= '"+_dateStart.fullYear+"-"+gnncDataNumber.__setZero(e)+"-31 23:59:59' AND MIX <> 'FINANCIAL_TRANS' 						) as VALUE_TOTAL_MONTH_"+e+"_TOTAL, ";
			}
			
			_sql += "" +
				"(Select round(SUM(VALUE_OUT_PAY),2) 	from dbd_financial where DATE_FINAL >= '"+_dateStart.fullYear+"-01-01 00:00:00' AND DATE_FINAL <= '"+_dateStart.fullYear+"-12-31 23:59:59' AND MIX <> 'FINANCIAL_TRANS' AND ID_GROUP = gro.ID	) as VALUE_TOTAL_MONTH_"+0+", "+
				"(Select round(SUM(VALUE_OUT_PAY),2) 	from dbd_financial where DATE_FINAL >= '"+_dateStart.fullYear+"-01-01 00:00:00' AND DATE_FINAL <= '"+_dateStart.fullYear+"-12-31 23:59:59' AND MIX <> 'FINANCIAL_TRANS'  						) as VALUE_TOTAL_MONTH_"+0+"_TOTAL "+
				
				"from "+ 
				"dbd_group as gro "+
				
				"where " +
				"gro.MIX = 'FINANCIAL_OUT' " + 
				"AND gro.MIX <> 'FINANCIAL_TRANS' " + 
				
				"order by "+
				"gro.MIX,gro.ID_FATHER asc,NAME_GROUP asc";
